...
{ 
	...
	//配置tim2触发频率为512Hz
	#if SINGLECHANNEL
	TIM_TimeBaseStructure.TIM_Period = TIM2_PERIOD - 1;							//设置512Hz采样频率
	#else
	TIM_TimeBaseStructure.TIM_Period = TIM2_PERIOD / ADC_CHANNEL_NUMS - 1;		//设置ADC_CHANNEL_NUMS * 512Hz采样频率
	#endif
...
void ADC_TIM3_Configuration(void)
{
	...
	#else
	TIM_TimeBaseInitStructure.TIM_Period = TIM3_PERIOD / ADC_CHANNEL_NUMS - 1;		//设置ADC_CHANNEL_NUMS * 512Hz采样频率
	#endif
