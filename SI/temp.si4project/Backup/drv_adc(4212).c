...
u16 ADC_SourceData[SAMPLS_NUM] = {0};
...
void ADC_Init_Configuration(void)//ADC配置函数
{
	...
	ADC_InitStructure.ADC_ScanConvMode       = DISABLE;					//如果是多通道，使能扫描模式
...
/******************************************************************************************************************************************
*adc初始化配置
*1.目的：用ADC采样值进行fft运算，求取采样后波形相关属性（本例主要分析50Hz正选波，及高/低次谐波）
*2.方案：计划ADC采样频率为512Hz，采样1024个点进行fft运算，使频域分辨率达到0.5Hz
*3.配置：由于ADC本身不能配置到512Hz，采用tim3定时触发采样
*4.其他：当系统时钟为72MHz时，ADC时钟最慢是9MHz，采样周期最大约为256个时钟周期，所以ADC本身采样频率最慢约为35156.25Hz（大于我需要的512Hz）
